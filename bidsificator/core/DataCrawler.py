...
class DataCrawler:
    def __init__(self, config_file):
        ...
        if 'data_paths' in self.config_data:
            # New format
            self.data_paths = self.config_data['data_paths']
            self.subject_pattern = self.config_data['subject_pattern']
            self.file_types = self.config_data['file_types']
        else:
            # Old format
            self.data_paths = self.config_data['db_path']
            self.subject_pattern = self.config_data['subject_structure']['subject_pattern']
            self.file_types = self.config_data['subject_structure']['data_types']
    ...
    def __get_subject_dirs(self):
        """Find subject directories based on the given base paths and pattern."""
        # Handle new format with data_paths
        if 'main' in self.data_paths:
            # Single database
            return self.__find_subject_dirs([self.data_paths['main']], self.subject_pattern)
        elif 'anatomical' in self.data_paths and 'functional' in self.data_paths:
            # Dual databases
            same_folder = self.data_paths['anatomical'] == self.data_paths['functional']
            if same_folder:
    ...
    def process_subject_data(self):
        ...
        for subject_dir in subject_dirs:
            ...
            for data_type, data_info in self.file_types.items():
                # Use 'folder' if available (new format), otherwise fall back to 'directory' (old format)
                data_type_dir = data_info.get('folder', data_info.get('directory', ''))
                ...
                for expanded_dir in expanded_dirs:
                    # Use 'extensions' if available (new format), otherwise fall back to 'file_extensions' (old format)
                    file_extensions = data_info.get('extensions', data_info.get('file_extensions', []))
