                ...
                for row in reader:
                    ...
                    if center_id_str.upper() == 'CUSTOM':
                        ...
                        if not re.match(r'^[a-zA-Z0-9]+$', subject_id_str):
                            errors.append(
                                f"Line {line_number}: SubjectID must be alphanumeric only, "
                                f"got '{subject_id_str}'"
                            )
                            continue
                        ...
                        try:
                            ...
                            errors.append(
                                f"Line {line_number}: CenterID must be numeric (0-999) or 'CUSTOM', "
                                f"got '{center_id_str}'"
                            )
                            continue
                        ...
                        try:
                            ...
                            errors.append(
                                f"Line {line_number}: SubjectID must be numeric when CenterID is numeric, "
                                f"got '{subject_id_str}'"
                            )
                            continue
                    ...
                    if not formatted_name:
                        if center_id_str.upper() == 'CUSTOM':
                            errors.append(
                                f"Line {line_number}: Could not create valid subject name from "
                                f"SubjectID='{subject_id_str}'"
                            )
                        else:
                            errors.append(
                                f"Line {line_number}: Could not create valid subject name from CenterID={center_id}, "
                    ...
                    if formatted_name in used_names:
                        if center_id_str.upper() == 'CUSTOM':
                            ...
                            errors.append(
                                f"Line {line_number}: Duplicate subject name '{formatted_name}' "
                                f"(from CenterID={center_id}, SubjectID={subject_id})"
                            )
                        continue
