...
class FileDetectionResult:
    ...
    source_path: Path
    detected_datatype: Optional[str]
    detected_suffix: Optional[str]
...
class FileDetectionService:
    ...
    def _detect_bids_info(self, file_path: Path) -> Tuple[Optional[str], Optional[str], float, List[str]]:
        """
        ...
        if best_confidence < 0.5:
            fallback_datatype, fallback_suffix, fallback_confidence, fallback_reasons = self._pattern_based_detection(file_path)
    ...
    def _pattern_based_detection(self, file_path: Path) -> Tuple[Optional[str], Optional[str], float, List[str]]:
    ...
    def get_modality_info(self, datatype: str) -> Optional[ModalityInfo]:
    ...
    def get_file_filters(self) -> Dict[str, str]:
        ...
        for ext, converter_names in converter_formats.items():
